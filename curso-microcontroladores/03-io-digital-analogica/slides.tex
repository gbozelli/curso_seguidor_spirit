...
\begin{frame}{Por que é importante entender o PWM para microcontroladores?}
  \begin{itemize}
    ...
    \item A saída de pinos PWM no osciloscópio não mostram uma tensão variável, mas sim uma onda retangular com largura de pulso variável. Se o motor não estiver respondendo como esperado, pode ser necessário verificar se o sinal PWM está correto.
    \item Mas se a saída é PWM, como ocorre essa transformação para o sinal semi-analógico que o motor recebe? O motor tem uma inércia mecânica que faz com que ele responda à média do sinal PWM ao longo do tempo. Quanto maior o ciclo de trabalho, mais tempo o motor recebe energia, e mais rápido ele gira. Quanto menor o ciclo de trabalho, menos energia é fornecida, e o motor gira mais lentamente.
  \end{itemize}
\end{frame}
...
\begin{frame}{Entendendo os argumentos de `ledcAttach()`}
  \begin{itemize}
    ...
    \item Frequência <1kHz: visível a olho nu (LED piscando).
